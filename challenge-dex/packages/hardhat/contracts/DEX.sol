...
contract DEX {
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        ...
        // Chuyển token BAL từ ví bạn vào hợp đồng DEX
    ...
    function withdraw(uint256 amount) public returns (uint256 ethAmount, uint256 tokenAmount) {
        ...
        uint256 tokenReserve = token.balanceOf(address(this));
        uint256 ethWithdrawn_;

        ethWithdrawn_ = amount * ethReserve / totalLiquidity;

        uint256 tokenAmount_ = amount * tokenReserve / totalLiquidity;
        liquidity[msg.sender] -= amount;
        totalLiquidity -= amount;
        (bool sent, ) = payable(msg.sender).call{ value: ethWithdrawn_ }("");
        require(sent, "withdraw(): revert in transferring eth to you!");
        require(token.transfer(msg.sender, tokenAmount_));
        emit LiquidityRemoved(msg.sender, amount, tokenAmount_, ethWithdrawn_);
        return (ethWithdrawn_, tokenAmount_);
    }
}
